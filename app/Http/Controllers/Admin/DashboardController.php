...
class DashboardController extends Controller
{
    ...
    {
        /**Coutn Registration */
        $response['signup_general'] = Signup::where('status','!=','RECUSADO')->count();

        $response['signup_invited'] = Signup::where([
            ['category','LIKE', '%'."CONVIDADO".'%'],
            ['status','!=','RECUSADO']

        ])->count();
        $response['signup_organizer'] = Signup::where([
                ['category','LIKE', '%'."ORGANIZAÇÃO".'%'],
                ['status','!=','RECUSADO']
             ])->count();

        $response['signup_press'] = Signup::where([

            ['category','LIKE', '%'."IMPRENSA".'%'],
            ['status','!=','RECUSADO']
        ]
        )->count();
        $response['signup_techinicalSuporter'] = Signup::where([
